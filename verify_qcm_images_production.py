...
def verify_qcm_images():
    """Vérifie que toutes les images des exercices QCM sont accessibles."""
    try:
...
def fix_qcm_images():
    """Corrige les problèmes d'images QCM"""
    print("=== Correction des problèmes d'images QCM ===\n")
    
    # 1. Créer les répertoires nécessaires
    ensure_directory_exists('static/uploads/exercises/qcm')
    ...
    # 4. Récupérer tous les exercices QCM
    try:
        cursor.execute("SELECT id, title, content, image_path FROM exercise WHERE exercise_type = 'qcm'")
        exercises = cursor.fetchall()
        logger.info(f"{len(exercises)} exercices QCM trouvés")
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des exercices QCM: {e}")
        return
    ...
    for exercise_id, title, content_json, db_image_path in exercises:
        ...
        try:
            ...
            if alt_path:
                ...
                target_path = 'static/uploads/exercises/qcm/' + filename
                shutil.copy2(alt_path, target_path)
                print(f"  ✅ Image copiée de {alt_path} vers {target_path}")
                
                # Mettre à jour le chemin dans la base de données
                new_path = '/static/uploads/exercises/qcm/' + filename
    ...
    print(f"{fixed_count} exercices QCM corrigés sur {len(exercises)}")
