...
class TestLegendImageUpload(TestCase):
    def create_app(self):
    ...
    def setUp(self):
        db.create_all()
        # Créer un exercice de test de type legend
        exercise = Exercise(
        ...
        self.test_image_path = os.path.join(app.config['UPLOAD_FOLDER'], 'test_legend_image.png')
    ...
    def create_test_image(self, path, width=400, height=300):
        ...
        d.text((10, 10), "Image de test pour Legend", fill=(255, 255, 0), font=font)
        d.text((10, 50), f"Timestamp: {time.time()}", fill=(255, 255, 0), font=font)
        
        # Dessiner quelques points pour les zones
        for i in range(5):
            x = random.randint(50, width-50)
            y = random.randint(50, height-50)
            d.ellipse((x-5, y-5, x+5, y+5), fill=(255, 0, 0))
            d.text((x+10, y), f"Point {i+1}", fill=(255, 255, 255), font=font)
            
        image.save(path)
        return path

    def test_legend_image_upload(self):
        """Test l'upload d'une image pour un exercice Legend"""
        ...
        with self.client as c:
            ...
            data = {
                'title': 'Test Legend Exercise Updated',
                'description': 'Updated description',
                'subject': 'Test',
                'legend_mode': 'classic',
                'legend_instructions': 'Updated instructions'
            }
            
            # Ajouter quelques zones pour le test
            for i in range(3):
                data[f'zone_{i}_x'] = str(100 + i*50)
                data[f'zone_{i}_y'] = str(100 + i*30)
                data[f'zone_{i}_legend'] = f'Test Legend {i+1}'
            
            # Créer un fichier en mémoire pour l'upload
            img_file = BytesIO(img_data)
            files = {
                'legend_main_image': (os.path.basename(self.test_image_path), img_file, 'image/png')
            }
    ...
    def test_legend_image_removal(self):
        """Test la suppression d'une image pour un exercice Legend"""
        # D'abord, uploader une image
        exercise, content = self.test_legend_image_upload()
        ...
        with self.client as c:
            ...
            data = {
                ...
                'legend_mode': 'classic',
                'legend_instructions': json.loads(exercise.content).get('instructions', ''),
            ...
            for i, zone in enumerate(content.get('zones', [])):
                data[f'zone_{i}_x'] = str(zone.get('x', 0))
                data[f'zone_{i}_y'] = str(zone.get('y', 0))
                data[f'zone_{i}_legend'] = zone.get('legend', '')
