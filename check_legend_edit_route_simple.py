...
def check_legend_edit_route():
    ...
    if os.path.exists(template_path):
        print(f"[OK] Le template {template_path} existe.")
    else:
    ...
    try:
        with open("app.py", "r", encoding="utf-8") as file:
            ...
            if "[LEGEND_EDIT_DEBUG] Traitement du contenu LÉGENDE" in app_content:
                print("\n[OK] La logique de debug pour les exercices de type légende est présente dans app.py.")
            else:
                print("\n[ATTENTION] La logique de debug pour les exercices de type légende n'a pas été trouvée dans app.py.")
            
            if "legend_mode = request.form.get('legend_mode', 'classic')" in app_content:
                print("[OK] La logique de traitement du mode de légende est présente dans app.py.")
            else:
                print("[ATTENTION] La logique de traitement du mode de légende n'a pas été trouvée dans app.py.")
            
            # Vérifier la présence de la gestion des trois modes de légende
            if "if legend_mode == 'grid':" in app_content:
                print("[OK] La gestion du mode quadrillage est présente.")
            else:
                print("[ATTENTION] La gestion du mode quadrillage n'a pas été trouvée.")
                
            if "if legend_mode == 'spatial':" in app_content:
                print("[OK] La gestion du mode spatial est présente.")
            else:
                print("[ATTENTION] La gestion du mode spatial n'a pas été trouvée.")
                
            # Vérifier la mise à jour du contenu de l'exercice
            if "exercise.content = json.dumps(content)" in app_content:
                print("[OK] La mise à jour du contenu de l'exercice est présente.")
            else:
                print("[ATTENTION] La mise à jour du contenu de l'exercice n'a pas été trouvée.")
    except Exception as e:
        print(f"[ERREUR] Erreur lors de la lecture de app.py: {e}")
