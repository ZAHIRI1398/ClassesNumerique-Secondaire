    ...
    with app.app_context():
        try:
            ...
            for exercise in exercises:
                ...
                attempts = Attempt.query.filter_by(exercise_id=exercise_id).order_by(Attempt.created_at.desc()).limit(3).all()
                if attempts:
                    print("\n  Tentatives récentes:")
                    for attempt in attempts:
                        created_at = attempt.created_at
                        print(f"  - ID: {attempt.id}, User: {attempt.user_id}, Score: {attempt.score}%, Date: {created_at.strftime('%Y-%m-%d %H:%M:%S')}")
...
if __name__ == "__main__":
    ...
    print("python diagnose_fill_in_blanks.py fix")
