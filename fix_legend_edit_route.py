...
"""
Script de correction pour la route d'édition des exercices de type légende
Ce script corrige l'erreur de serveur interne (500) qui se produit lors de la modification
d'un exercice de type légende en implémentant correctement la logique de traitement
...
def fix_legend_edit_route(app_path):
    """Corrige la route d'édition des exercices de type légende dans app.py"""
    with open(app_path, 'r', encoding='utf-8') as file:
        content = file.read()
    
    # Recherche de la section à corriger dans la route edit_exercise_blueprint
    pattern = r'(if exercise\.exercise_type == \'legend\':\s*?print\(f\'\[LEGEND_EDIT_DEBUG\] Traitement du contenu LÉGENDE\'\).*?)(# Sauvegarder les modifications\s*?db\.session\.commit\(\)\s*?flash\(\'Exercice modifié avec succès !\', \'success\'\)\s*?return redirect\(url_for\(\'view_exercise\', exercise_id=exercise_id\)\)\s*?\})'
    
    # Code de remplacement avec la logique complète de traitement des exercices de type légende
    replacement = r"""\1
                ...
                zone_indices = sorted(zone_indices)
                print(f'[LEGEND_EDIT_DEBUG] Zones trouvées: {zone_indices}')
    ...
    if updated_content == content:
        print("ATTENTION: Aucune modification n'a été appliquée. Le pattern n'a pas été trouvé.")
        return False
    ...
    print("Correction appliquée avec succès à la route d'édition des exercices de type légende.")
    return True
...
def main():
    print("=== Correction de la route d'édition des exercices de type légende ===\n")
    ...
    success = fix_legend_edit_route(app_path)
