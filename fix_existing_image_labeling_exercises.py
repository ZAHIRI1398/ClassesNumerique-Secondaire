"""
Script pour corriger les chemins d'images dans les exercices existants de type image_labeling.
...
def fix_image_labeling_exercises(db_path, exercise_id=None):
    """
    Corrige les chemins d'images dans les exercices de type image_labeling.
    
    Args:
        db_path (str): Chemin vers le fichier de base de données SQLite
        exercise_id (int, optional): ID de l'exercice à corriger. Si None, corrige tous les exercices.
    ...
    if exercise_id:
        query = "SELECT * FROM exercise WHERE exercise_type = 'image_labeling' AND id = ?"
        params = (exercise_id,)
    else:
...
def main():
    ...
    # Demander à l'utilisateur s'il veut corriger un exercice spécifique ou tous
    choice = input("Voulez-vous corriger un exercice spécifique? (o/n): ").lower()
    
    if choice == 'o':
        exercise_id = input("Entrez l'ID de l'exercice à corriger: ")
        try:
            exercise_id = int(exercise_id)
            stats = fix_image_labeling_exercises(db_path, exercise_id)
            print(f"[SUCCESS] Correction terminee pour l'exercice {exercise_id}.")
        except ValueError:
            print("[ERROR] L'ID de l'exercice doit etre un nombre entier.")
            return
    else:
        stats = fix_image_labeling_exercises(db_path)
        print("[SUCCESS] Correction terminee pour tous les exercices de type image_labeling.")
    
    # Afficher les statistiques
    print("[INFO] Statistiques:")
    print(f"- Total d'exercices traités: {stats['total']}")
    print(f"- Exercices avec image_path corrigé: {stats['fixed_image_path']}")
    print(f"- Exercices avec main_image corrigé: {stats['fixed_content_image']}")
    print(f"- Exercices déjà corrects: {stats['already_correct']}")
