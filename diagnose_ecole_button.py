...
def check_template_content():
    ...
    with open(template_path, 'r', encoding='utf-8') as f:
        ...
        if login_modal_ecole:
            logger.info("✅ Le code du bouton École est présent dans le template.")
            ...
            logger.error("❌ Le code du bouton École n'est pas présent dans le template!")
            return False
...
def check_route_exists():
    ...
    if not os.path.exists('app.py'):
        logger.error("❌ Le fichier app.py n'existe pas!")
        return False
    ...
    with open('app.py', 'r', encoding='utf-8') as f:
        ...
        if "@app.route('/register/school'" in content and "def register_school():" in content:
            logger.info("✅ La route register_school est définie dans app.py")
            return True
        else:
            logger.error("❌ La route register_school n'est pas définie dans app.py!")
            return False
...
def test_template_rendering():
    ...
    try:
        ...
        if '/register/school' in rendered and 'École' in rendered and 'fa-school' in rendered:
            logger.info("✅ Le bouton École est présent dans le rendu du template.")
            return True
        else:
            logger.error("❌ Le bouton École n'est pas présent dans le rendu du template!")
            return False
    ...
    try:
        ...
        with app.test_request_context():
            try:
                ...
                logger.info(f"✅ L'URL pour register_school est: {url}")
            except Exception as e:
                logger.error(f"❌ Erreur lors de la génération de l'URL pour register_school: {str(e)}")
                return False
        
        logger.info("✅ La configuration Flask semble correcte.")
        return True
    except Exception as e:
        logger.error(f"❌ Erreur lors de la vérification de la configuration Flask: {str(e)}")
        return False

def check_browser_compatibility():
    """Vérifie les potentiels problèmes de compatibilité navigateur"""
    logger.info("ℹ️ Vérification de la compatibilité navigateur:")
