...
# Table d'association entre Class et Exercise
class_exercise = db.Table('class_exercise',
    db.Column('class_id', db.Integer, db.ForeignKey('class.id'), primary_key=True),
    db.Column('exercise_id', db.Integer, db.ForeignKey('exercise.id'), primary_key=True)
)

# Classe pour la compatibilité avec le code existant
class ClassExercise:
    """Classe utilitaire pour faciliter l'accès à la table class_exercise"""
    @staticmethod
    def query():
        return db.session.query(class_exercise)
    ...
    def get_content(self):
        ...
        try:
            ...
            # Pour les exercices de type fill_in_blanks
                ...
                if 'sentences' not in content or 'words' not in content:
                    content['sentences'] = []
                    content['words'] = []
                
                # S'assurer que les phrases sont des chaînes simples
                sentences = []
            ...
            # Pour les exercices de type pairs
                ...
                if 'pairs' not in content and 'left_items' not in content:
                    content['pairs'] = []
            
                # Validation de la structure des paires si elles existent
                if 'pairs' in content:
                    for pair in content['pairs']:
                        if isinstance(pair, dict):
                            # S'assurer que chaque paire a les bonnes clés
                            if 'left' not in pair:
                                pair['left'] = {'content': '', 'type': 'text'}
                            if 'right' not in pair:
                                pair['right'] = {'content': '', 'type': 'text'}
                        
                            # S'assurer que left et right sont des dictionnaires
                            if not isinstance(pair['left'], dict):
            ...
            # Pour les exercices de type QCM
                ...
                for question in content['questions']:
                    if not isinstance(question, dict):
                        continue
                    # Copier 'question' vers 'text' si présent
                    if 'question' in question and not question.get('text'):
                        question['text'] = question['question']
                    if 'text' not in question:
                        question['text'] = ''
                    if 'options' not in question:
                        question['options'] = []
                    # Copier 'correct' vers 'correct_answer' si présent
                    if 'correct' in question:
                        question['correct_answer'] = str(question['correct'])
