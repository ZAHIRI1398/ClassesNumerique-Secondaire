...
def check_app_py():
    ...
    try:
        ...
        if match_first_impl:
            print("✅ La première implémentation est bien désactivée avec un 'pass'")
            ...
            if "user_answers" in disabled_section or "correct_answers" in disabled_section:
                print("❌ PROBLÈME: Il y a encore du code actif après le 'pass'")
                print("Extrait du code problématique:")
                print(disabled_section[:200] + "..." if len(disabled_section) > 200 else disabled_section)
            else:
                print("✅ Pas de code actif après le 'pass'")
        else:
            print("❌ La première implémentation n'est pas correctement désactivée")
        ...
        if match_second_impl:
            print("✅ La deuxième implémentation est présente")
            ...
            if re.search(pattern_score_calc, content):
                print("✅ Le calcul du score est correct")
            else:
                print("❌ Le calcul du score n'est pas trouvé ou incorrect")
        else:
            print("❌ La deuxième implémentation n'est pas trouvée")
        ...
        if match_redirect:
            print("ℹ️ La soumission redirige vers view_exercise")
...
def check_process_running():
    ...
    try:
            ...
            try:
                ...
                if cmdline and 'python' in proc.info['name'].lower():
                    ...
                    if 'app.py' in cmdline_str or 'flask run' in cmdline_str:
                        print(f"\n✅ Processus Flask trouvé (PID: {proc.pid})")
        ...
        print("\n❌ Aucun processus Flask trouvé en cours d'exécution")
        print("⚠️ L'application doit être redémarrée pour appliquer les modifications")
        return False
    except ImportError:
        print("\n❌ Module psutil non disponible, impossible de vérifier les processus")
