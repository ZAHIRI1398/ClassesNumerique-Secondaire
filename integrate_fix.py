...
def integrate_fix_blueprint():
    """Intègre le blueprint de correction dans app.py"""
    app_path = "app.py"
    ...
    try:
        ...
        if "from fix_school_choice import fix_bp" in content:
            print("Le blueprint de correction est déjà importé.")
            return False
        ...
        print("Blueprint de correction intégré avec succès dans app.py")
        return True
...
def main():
    """Fonction principale"""
    print("=== INTÉGRATION DE LA CORRECTION DU CHOIX D'ÉCOLE ===")
    ...
    if not os.path.exists("fix_school_choice.py"):
        print("Erreur: Le fichier fix_school_choice.py n'existe pas dans le répertoire courant.")
        return
    ...
    if integrate_fix_blueprint():
        print("\nIntégration réussie!")
        print("Vous pouvez maintenant accéder à /fix/school-choice pour corriger le problème de choix d'école.")
