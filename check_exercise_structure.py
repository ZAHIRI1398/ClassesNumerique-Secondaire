        ...
        try:
            ...
            if 'sentences' in content:
                ...
                if isinstance(content['sentences'], list):
                    print("✓ 'sentences' est une liste")
                    
                    for i, sentence in enumerate(content['sentences']):
                        print(f"\nPhrase {i+1}:")
                        if isinstance(sentence, dict):
                            print("✓ La phrase est un dictionnaire")
                            if 'text' in sentence:
                                print(f"✓ Clé 'text' présente: {sentence['text']}")
                            else:
                                print("✗ Clé 'text' manquante")
                            
                            if 'answer' in sentence:
                                print(f"✓ Clé 'answer' présente: {sentence['answer']}")
                            else:
                                print("✗ Clé 'answer' manquante")
                        else:
                            print(f"✗ La phrase est de type {type(sentence).__name__}, mais devrait être un dictionnaire")
                            print(f"  Valeur: {sentence}")
                else:
                    print(f"✗ 'sentences' est de type {type(content['sentences']).__name__}, mais devrait être une liste")
            else:
                print("✗ Clé 'sentences' manquante dans le contenu")
            
            # Vérifier la présence de 'words'
            if 'words' in content:
                print("\nVérification de la structure 'words':")
                if isinstance(content['words'], list):
                    print(f"✓ 'words' est une liste: {content['words']}")
                else:
                    print(f"✗ 'words' est de type {type(content['words']).__name__}, mais devrait être une liste")
            else:
                print("✗ Clé 'words' manquante dans le contenu")
            ...
            print("\nVoulez-vous corriger la structure de l'exercice? (y/n)")
            choice = input().strip().lower()
            
            if choice == 'y':
                exercise.content = json.dumps(corrected_content, ensure_ascii=False)
                db.session.commit()
                print("✓ Exercice mis à jour avec la structure corrigée")
            else:
                print("Aucune modification apportée à l'exercice")
